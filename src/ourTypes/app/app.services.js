"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
require("rxjs/add/operator/map");
require("rxjs/add/operator/take");
var Appservices = (function () {
    function Appservices(_http) {
        this._http = _http;
    }
    Appservices.prototype.getPosts = function () {
        return this._http.get("https://api.github.com/users")
            .map(function (res) { return res.json(); })
            .take(2);
    };
    Appservices.prototype.getUser = function (userLogin) {
        return this._http.get("https://api.github.com/users/" + userLogin)
            .map(function (res) { return res.json(); });
    };
    Appservices.prototype.getSpecificUser = function (id) {
        return this._http.get("https://api.github.com/users/" + id)
            .map(function (res) { return res.json(); });
    };
    return Appservices;
}());
Appservices = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], Appservices);
exports.Appservices = Appservices;
//# sourceMappingURL=app.services.js.map